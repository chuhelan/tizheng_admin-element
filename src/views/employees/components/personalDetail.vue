  ...
  methods: {
    ...
    async savePersonal() {
      ...
      await updatePersonal({
        ...
        id: this.userId,
        staffPhoto:
          this.$refs.userAvator.fileList.length &&
          this.$refs.userAvator.fileList
            ? this.$refs.info.fileList[0].url
            : ' '
      })
    ...
    async saveUser() {
      //  调用父组件
      const flag = this.$refs.info.fileList.some(
        (item) => item.upload === true
      )
      if (!flag) {
        this.$message.error('头像还未上传成功')
      ...
      await saveUserDetailById({
        ...this.userInfo,
        staffPhoto:
          this.$refs.info.fileList.length && this.$refs.info.fileList.length
            ? this.$refs.info.fileList[0].url
            : ' '
      })
      this.$message.success('保存成功')
